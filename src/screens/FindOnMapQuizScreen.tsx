...
export function FindOnMapQuizScreen({ onBack }: { onBack: () => void }) {
  ...
  const [guessedIds, setGuessedIds] = React.useState<Set<number>>(
    () => new Set()
  );
  ...
  const pickNext = React.useCallback(
    (currentGuessedIds?: Set<number>) => {
      ...
      const remaining = playable.filter(
        (p) => !activeGuessedIds.has(p.idNumeric)
      );
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 items-start">
            ...
            <Card className="relative overflow-hidden h-[65vh]">
              <div className="absolute top-4 left-4 z-10">
                <span className="inline-flex items-center gap-2 bg-background/80 backdrop-blur-sm px-3 py-1.5 rounded-full border shadow-sm text-sm font-medium">
                  ...
                  {feedback === "correct" && (
                    <span className="text-emerald-600 flex items-center gap-1 font-bold animate-bounce">
                      <CheckCircle2Icon className="size-4" /> 정답입니다!
                    </span>
                  )}
                  {feedback === "wrong" && (
                    <span className="text-rose-600 flex items-center gap-1 font-bold animate-shake">
                      <XCircleIcon className="size-4" /> 틀렸습니다!
                    </span>
                  )}
                  {feedback === "failed" && (
                    <span className="text-rose-700 flex items-center gap-1 font-bold">
                      <XCircleIcon className="size-4" /> 게임 종료
                    </span>
                  )}
                </span>
              </div>
                      ...
                      <Geographies geography={worldTopo}>
                        {({ geographies }: { geographies: any[] }) =>
                          geographies.map((geo: any) => {
                            ...
                            const isTarget =
                              feedback === "failed" &&
                              idNumeric === targetCountry?.idNumeric;
            ...
            <Card className="h-fit">
              <CardHeader>
                <CardTitle className="text-lg">게임 방법</CardTitle>
              </CardHeader>
              <CardContent className="text-sm text-muted-foreground space-y-4">
                <p>
                  1. 화면 상단에 제시된 <strong>국기</strong>와{" "}
                  <strong>이름</strong>을 확인하세요.
                </p>
                <p>
                  2. 지도에서 해당 국가의 영역을 찾아 <strong>클릭</strong>
                  하세요.
                </p>
                <p>
                  3. 총 <strong>3번의 기회</strong>가 주어지며, 모두 소진하면
                  게임이 종료됩니다.
                </p>
          ...
          <Card className="w-full max-w-md shadow-2xl border-primary/20 scale-in-center">
            ...
            <CardContent className="space-y-8 text-center pt-4">
              ...
              <div className="flex flex-col gap-3">
                <Button
                  size="lg"
                  className="w-full text-lg h-14 font-bold rounded-xl shadow-lg shadow-primary/20"
                  onClick={onReset}
                >
